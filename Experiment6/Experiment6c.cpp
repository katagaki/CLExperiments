...
int main(void) {
    ...
    cl::Image2D image1Buffer;
    cl::Image2D image2Buffer;
    cl::Image2D image3Buffer;
    ...
    std::cout << "Reading Experiment6c.cl...";
    programFile = std::ifstream("Experiment6c.cl");
    ...
    std::cout << "Compiling Experiment6c.cl...";
    try {
    ...
    // Read file
    std::cout << "Enter the name of the file containing the image to apply the bloom effect: ";
    std::cin >> inputFileName;
    originalImage = read_BMP_RGB_to_RGBA(inputFileName, &imageWidth, &imageHeight);
    ...
    thresholdImage = new unsigned char[imageWidth * imageHeight * 4];
    
    // Prepare buffers
    std::cout << "Preparing buffers...";
    image1Buffer = cl::Image2D(defaultContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)originalImage);
    image2Buffer = cl::Image2D(defaultContext, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)thresholdImage);
    std::cout << " done." << std::endl << std::endl;
    
    // Ask user for luminance
    std::cout << "Enter the luminance to use (between 0 and 1): ";
    std::cin >> luminance;
    std::cout << std::endl;
    
    // Enqueue kernel
    std::cout << "Enqueuing kernels now!" << std::endl << std::endl;
    kernels[0].setArg(0, image1Buffer);
    kernels[0].setArg(1, image2Buffer);
    kernels[0].setArg(2, luminance);
    ...
    region[2] = 1;
    defaultQueue.enqueueReadImage(image2Buffer, CL_TRUE, origin, region, 0, 0, thresholdImage);
    
    // Write file
    outputHasError = write_BMP_RGBA_to_RGB("Threshold.bmp", thresholdImage, imageWidth, imageHeight);
    if (!outputHasError) {
        return 20;
    } else {
        std::cout << "Threshold.bmp saved successfully." << std::endl;
    }
    
    // Set the size of the blurred output
    blurredImage = new unsigned char[imageWidth * imageHeight * 4];
    
    // Prepare buffers
    std::cout << "Preparing buffers...";
    image1Buffer = cl::Image2D(defaultContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)thresholdImage);
    image2Buffer = cl::Image2D(defaultContext, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)blurredImage);
    std::cout << " done." << std::endl << std::endl;
    
    // Enqueue kernel
    std::cout << "Enqueuing kernels now!" << std::endl << std::endl;
    kernels[1].setArg(0, image1Buffer);
    kernels[1].setArg(1, image2Buffer);
    hasError = defaultQueue.enqueueNDRangeKernel(kernels[1], cl::NDRange(0, 0), cl::NDRange(imageWidth, imageHeight));
    ...
    region[1] = imageHeight;
    region[2] = 1;
    defaultQueue.enqueueReadImage(image2Buffer, CL_TRUE, origin, region, 0, 0, blurredImage);
    
    // Write file
    outputHasError = write_BMP_RGBA_to_RGB("Blur.bmp", blurredImage, imageWidth, imageHeight);
    if (!outputHasError) {
        return 20;
    } else {
        std::cout << "Blur.bmp saved successfully." << std::endl;
    }
    
    // Set the size of the blurred output
    finalImage = new unsigned char[imageWidth * imageHeight * 4];
    
    // Prepare buffers
    std::cout << "Preparing buffers...";
    image1Buffer = cl::Image2D(defaultContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)blurredImage);
    image2Buffer = cl::Image2D(defaultContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)originalImage);
    image3Buffer = cl::Image2D(defaultContext, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, cl::ImageFormat(CL_RGBA, CL_UNORM_INT8), imageWidth, imageHeight, 0, (void*)finalImage);
    std::cout << " done." << std::endl << std::endl;
    
    // Enqueue kernel
    std::cout << "Enqueuing kernels now!" << std::endl << std::endl;
    kernels[2].setArg(0, image1Buffer);
    kernels[2].setArg(1, image2Buffer);
    kernels[2].setArg(2, image3Buffer);
    hasError = defaultQueue.enqueueNDRangeKernel(kernels[2], cl::NDRange(0, 0), cl::NDRange(imageWidth, imageHeight));
    ...
    region[1] = imageHeight;
    region[2] = 1;
    defaultQueue.enqueueReadImage(image3Buffer, CL_TRUE, origin, region, 0, 0, finalImage);
    
    // Write file
    outputHasError = write_BMP_RGBA_to_RGB("Bloom.bmp", finalImage, imageWidth, imageHeight);
    if (!outputHasError) {
        return 20;
    } else {
        std::cout << "Bloom.bmp saved successfully." << std::endl;
    }
