...
int main(void) {
    ...
    std::cout << "Reading Experiment5a.cl...";
    programFile = std::ifstream("Task2a.cl");
    ...
    std::cout << "Compiling Experiment5a.cl...";
    try {
    ...
    transform(ptString.begin(), ptString.end(), ptString.begin(), ::toupper);
    ...
    std::cout << "Enter the shift value for the shift cipher (between 1 to 26): ";
    std::cin >> shiftValue;
    if (shiftValue <= -26 || shiftValue >= 26 || shiftValue == 0) {
    ...
    kernels[0].setArg(2, shiftValue);
    hasError = defaultQueue.enqueueNDRangeKernel(kernels[0], 0, plainText.size() / 4);
